...
import { createVehicleCover, CoverState } from "@/lib/actions/vehicle-policy";
...
const benefitsConfig: Record<string, Benefit[]> = {
  ...
  "Motor Commercial (TPO)": [],
  "PSV Chauffeur Driven (TPO)": [{ name: "PLL", amount: 500, seats: 0 }],
  "Motor Cycle Private": [
    { name: "Excess Protector-OD", rate: "", value: 0 },
    { name: "PVTR", rate: "", value: 0 },
  ],
...
export default function PolicyForm({ vehicleId, covers, vehicle }: Props) {
  ...
  const assignInsurance = createVehicleCover.bind(
    null,
    ...
    if (coverName && benefitsConfig[coverName]) {
      const defaultBenefits = benefitsConfig[coverName].map((benefit: any) => {
        if (
          ...
          return { ...benefit, value: parseFloat(vehicle.valuation) };
        }
        return benefit;
      });
    ...
    <form ref={ref} action={formAction} className="flex flex-col gap-2 w-full">
      ...
      <div className="flex flex-col gap-2">
        <div>
          ...
          <p className="text-xs">{coverName}</p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {coverName === "PSV Chauffeur Driven (TPO)" ? (
